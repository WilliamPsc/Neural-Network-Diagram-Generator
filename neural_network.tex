...
\begin{standalonepage}
    \begin{tikzpicture}[
        ...
        % Weights signs
        \def\positiveWeights{1, 2, 4, 5, 6, 9, 10, 12, 13}
        \def\negativeWeights{0, 3, 7, 8, 11}
        \def\falseNegativeWeights{}

        % Values of all model weights
        \def\weightValues{-0.663758, 0.801922, 0.993988, -0.763415, 0.211472, 0.181081, 1.139164, -0.898664, -0.994529, 0.454182, 0.36435, -0.304097, 1.132819, 0.787494}
        \def\biasValues{}

        % Display the index number or not (0 no / 1 yes)
        \pgfmathsetmacro{\displayIndex}{1}
        \pgfmathsetmacro{\displayWeightValue}{1} % if set to 0, a name will be given instead of the value

        % Display bias on each neurons
        \pgfmathsetmacro{\displayBias}{0}
        ...
        \newcounter{nbParameters}
        \setcounter{nbParameters}{0}
        ...
        \foreach \layer in {1,...,\numHiddenLayers} {
            ...
            \foreach \i in {1,...,\numNeurons} {
                \ifnum\layer=1 % if you need another color for a specific layer (you need to specify the different color for each layer)
